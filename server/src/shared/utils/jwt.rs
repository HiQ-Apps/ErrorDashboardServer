...
use shared_types::auth_dtos::{Claims, RefreshTokenDTO};
...
pub async fn validate_namespace_secret_jwt(
    client_id: Uuid,
...
pub fn extract_user_id_from_jwt_header(
    headers: &HeaderMap,
    secret_key: &str,
    ...
    if let Some(token_header) = headers.get("Authorization") {
        ...
        let token_data: TokenData<Claims> =
            decode(token_str, &decoding_key, &Validation::default())
...
pub fn extract_user_id_from_jwt_cookie(
    cookies: &Cookie,
    secret_key: &str,
) -> Result<Uuid, ServerError> {
...
pub fn create_refresh_token(
    user_id: Uuid,
    configs: &Config,
) -> Result<RefreshTokenDTO, ServerError> {
    let jwt_iss = configs.jwt_issuer.clone();
    let jwt_aud = configs.jwt_audience.clone();
    let secret_key = &configs.secret_key;
    let now: DateTime<Utc> = Utc::now();
...
pub async fn refresh_access_token_util(
