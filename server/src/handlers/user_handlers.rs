...
use shared_types::user_dtos::{
    PasswordDTO, ResetPasswordPath, ResetPasswordRequestDTO, UpdateUserProfileDTO,
};
...
impl UserHandler {
    ...
    pub async fn update_password(
        ...
        match user_services
            .update_password(user_id, email, password)
            .await
        {
            Ok(()) => Ok(HttpResponse::Ok().finish()),
            Err(err) => Err(err),
    ...
    pub async fn update_user_profile(
        ...
        match user_services
            .update_user_profile(user_id.into_inner(), update_user_profile_dto)
            .await
        {
            Ok(user_profile) => Ok(HttpResponse::Ok().json(user_profile)),
            Err(err) => Err(err),
