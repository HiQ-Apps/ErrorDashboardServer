...
use shared_types::extra_dtos::{ErrorQueryParams, PaginationParams};
use shared_types::namespace_dtos::{
    CreateNamespaceDTO, InviteUserRequestDTO, UpdateNamespaceDTO, UpdateUserRoleRequestDTO,
};
...
impl NamespaceHandler {
    pub async fn create_namespace(
        ...
        let CreateNamespaceDTO {
            service_name,
            environment_type,
        } = new_namespace.into_inner();
        match namespace_services
            .create_namespace(
                user_id,
                service_name,
                environment_type,
    ...
    pub async fn get_namespaces_by_user_id(
        ...
        match namespace_services
            .get_namespaces_by_user_id(*user_id, offset, limit)
            .await
        {
    ...
    pub async fn update_namespace(
        ...
        match namespace_services
            .update_namespace(user_id, update_namespace_dto, role_rules.clone())
            .await
        {
    ...
    pub async fn delete_namespace(
        ...
        match namespace_services
            .delete_namespace(*namespace_id, user_id, notification_manager)
            .await
        {
    ...
    pub async fn get_errors_by_namespace_with_pagination(
        ...
        let result = namespace_services
            .get_errors_by_namespace_with_pagination(
                *namespace_id,
                group_by,
    ...
    pub async fn namespace_error_ws_session(
        ...
        actix_web::rt::spawn(namespace_error_ws_session(
            session,
            namespace_id,
            namespace_server,
        ));
    ...
    pub async fn invite_user_to_namespace(
        ...
        let namespace_id = namespace_id.into_inner();
        match namespace_services
            .invite_user_to_namespace(namespace_id, invite_user_data)
            .await
        {
            Ok(()) => Ok(HttpResponse::Ok().finish()),
            Err(err) => Err(err),
    ...
    pub async fn remove_user_from_namespace(
        ...
        match namespace_services
            .check_namespace_membership(deleter_id, namespace_id)
            .await
        {
            Ok(res) => {
                if !res {
        ...
        match namespace_services
            .remove_user_from_namespace(deletee_id, namespace_id)
            .await
        {
            Ok(()) => Ok(HttpResponse::Ok().finish()),
            Err(err) => Err(err),
    ...
    pub async fn get_user_role_in_namespace(
        ...
        let namespace_id = namespace_id.into_inner();
        match namespace_services
            .get_user_role_in_namespace(user_id, namespace_id)
            .await
        {
    ...
    pub async fn update_user_role_in_namespace(
        ...
        match namespace_services
            .check_user_namespace_perms(user_id, namespace_id, role_rules_ref, Permission::Update)
            .await
        {
            Ok(res) => {
                if !res {
        ...
        match namespace_services
            .compare_user_namespace_perms(user_id, target_user, namespace_id, role_rules_ref)
            .await
        {
            Ok(res) => {
                if !res {
        ...
        let new_role_enum = string_to_role(&new_role)
            .ok_or_else(|| ServerError::QueryError(QueryError::InvalidRole))?;

        let new_role_weight = get_weight(&new_role_enum, &role_rules).unwrap();
        let user_role = match namespace_services
            .get_user_role_in_namespace(user_id, namespace_id)
            .await
        {
            Ok(role) => role,
        ...
        let user_role_enum = string_to_role(&user_role)
            .ok_or_else(|| ServerError::QueryError(QueryError::InvalidRole))?;
        ...
        match namespace_services
            .update_user_role_in_namespace(target_user, namespace_id, new_role)
            .await
        {
            Ok(()) => Ok(HttpResponse::Ok().finish()),
            Err(err) => Err(err),
