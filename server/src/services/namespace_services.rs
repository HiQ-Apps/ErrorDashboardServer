...
use crate::models::user_namespace_junction_model::{
    Entity as UserNamespaceJunctionEntity, Model as UserNamespaceJunctionModel,
};
...
use shared_types::error_dtos::{
    AggregateIndividualErrorDTO, AggregatedResult, GetAggregatedLineErrorDTO,
    GetAggregatedMessageErrorDTO, GetAggregatedStatusErrorDTO, TagAggregatedErrorDTO,
};
use shared_types::namespace_dtos::{
    GetNamespaceResponseDTO, GetNamespacesByUserResponseDTO, InviteUserRequestDTO,
    ShortNamespaceDTO, UpdateNamespaceDTO,
};
...
impl NamespaceService {
    ...
    pub async fn get_all_namespaces(&self) -> Result<Vec<ShortNamespaceDTO>, ServerError> {
        ...
        let short_namespaces = namespaces
            .into_iter()
            .map(|namespace| ShortNamespaceDTO {
                id: namespace.id,
    ...
    pub async fn create_namespace(
        ...
        if user_namespace_count >= 10 {
            return Err(ServerError::RequestError(
                RequestError::NamespaceLimitReached,
            ));
        }
        ...
        if let Err(err) = UserNamespaceJunctionEntity::insert(user_namespace_junction)
            .exec(&transaction)
            .await
        {
            transaction.rollback().await.map_err(ExternalError::from)?;
        ...
        notification_manager
            .broadcast_notification(broadcast_notification, &user_id)
            .await;
    ...
    pub async fn get_namespace_by_id(
        &self,
        uid: Uuid,
    ) -> Result<GetNamespaceResponseDTO, ServerError> {
        let config = &*self.configs;
        let result = NamespaceEntity::find()
            .filter(<NamespaceEntity as sea_orm::EntityTrait>::Column::Id.eq(uid))
            .one(&*self.db)
            .await;
    ...
    pub async fn get_namespaces_by_user_id(
        ...
        offset: u64,
        limit: u64,
    ) -> Result<Vec<GetNamespacesByUserResponseDTO>, ServerError> {
        let junction_result = UserNamespaceJunctionEntity::find()
            .filter(
                <UserNamespaceJunctionEntity as sea_orm::EntityTrait>::Column::UserId.eq(user_id),
            )
            .all(&*self.db)
            .await;
        ...
        match junction_result {
            Ok(junctions) => {
                if junctions.is_empty() {
                    ...
                    let namespace_ids: Vec<Uuid> =
                        junctions.iter().map(|junc| junc.namespace_id).collect();
                    let namespaces = NamespaceEntity::find()
                        .filter(
                            <NamespaceEntity as sea_orm::EntityTrait>::Column::Id
                                .is_in(namespace_ids),
                        )
                        .offset(offset)
                        .limit(limit)
                    ...
                    let short_namespaces = namespaces
                        .into_iter()
                        .map(|namespace| GetNamespacesByUserResponseDTO {
                            id: namespace.id,
                            active: namespace.active,
                            service_name: namespace.service_name,
                            environment_type: namespace.environment_type,
                            role: junctions
                                .iter()
                                .find(|junc| junc.namespace_id == namespace.id)
                                .unwrap()
                                .role
                                .clone(),
                        })
        ...
        let namespace_junc = match namespace_junc_result {
            ...
            Ok(None) => {
                ...
                transaction
                    .rollback()
                    .await
                    .map_err(|err| ServerError::from(ExternalError::from(err)))?;
                return Err(ServerError::QueryError(
    ...
    pub async fn delete_namespace(
        ...
        let namespace_junc_result = UserNamespaceJunctionEntity::find()
            .filter(
                <UserNamespaceJunctionEntity as EntityTrait>::Column::NamespaceId.eq(namespace_id),
            )
            .filter(<UserNamespaceJunctionEntity as EntityTrait>::Column::UserId.eq(user_id))
            .one(&transaction)
            .await;
        ...
        for user in subscribed_users {
            ...
            notification_manager
                .broadcast_notification(notification_clone, &user.user_id)
                .await;
        }
    ...
    async fn aggregate_errors(
        ...
        let mut grouped_errors_by_message: HashMap<String, GetAggregatedMessageErrorDTO> =
            HashMap::new();
        ...
        if let Some(tags) = &error.tags {
            for tag in tags {
                let entry =
                    tag_aggregations
                        .entry(tag.clone())
                        .or_insert_with(|| TagAggregatedErrorDTO {
                            tag: tag.clone(),
                ...
                let user_set = unique_users
                    .entry(format!("{}:{}", tag.tag_key, tag.tag_value))
    ...
    fn aggregate_by_other(
        ...
        match group_by {
            "message" => {
                ...
                let entry = grouped_errors_by_message
                    .entry(key.clone())
                    .or_insert_with(|| GetAggregatedMessageErrorDTO {
                        message: error.message.clone(),
                        user_affected_count: 0,
                        error_count: 0,
                        aggregated_tags: vec![],
                ...
                if let Some(tags) = &error.tags {
                    let tag_set: HashSet<ShortTagNoIdDTO> =
                        entry.aggregated_tags.iter().cloned().collect();
                    let new_tags: HashSet<ShortTagNoIdDTO> = tags.iter().cloned().collect();
                    let combined_tags: HashSet<ShortTagNoIdDTO> =
                        tag_set.union(&new_tags).cloned().collect();
                    entry.aggregated_tags = combined_tags.into_iter().collect();
                }
            }
            "line" => {
                let key = error.line.clone();
                let entry = grouped_errors_by_line
                    .entry(key.to_string())
                    .or_insert_with(|| GetAggregatedLineErrorDTO {
    ...
    pub async fn get_errors_by_namespace_with_pagination(
        ...
        match group_by.as_str() {
            "tags" => {
                let mut tag_results: Vec<TagAggregatedErrorDTO> =
                    tag_aggregations.into_iter().map(|(_, v)| v).collect();
            ...
            "message" => {
                let mut message_results: Vec<GetAggregatedMessageErrorDTO> =
                    grouped_errors_by_message
                        .into_iter()
                        .map(|(_, v)| v)
                        .collect();
                message_results.sort_by(|a, b| b.error_count.cmp(&a.error_count));
                let paginated_results = message_results
                    .into_iter()
                    .skip(offset)
                    .take(limit)
                    .collect();
            ...
            "line" => {
                let mut line_results: Vec<GetAggregatedLineErrorDTO> =
                    grouped_errors_by_line.into_iter().map(|(_, v)| v).collect();
    ...
    pub async fn invite_user_to_namespace(
        &self,
        namespace_id: Uuid,
        invite_data: InviteUserRequestDTO,
    ) -> Result<(), ServerError> {
        let db = &*self.db;
        ...
        if let Err(err) =
            UserNamespaceJunctionEntity::insert(user_namespace_junction.into_active_model())
                .exec(db)
                .await
        {
            return Err(ServerError::from(ExternalError::from(err)));
    ...
    pub async fn check_user_namespace_perms(
        &self,
        user_id: Uuid,
        namespace_id: Uuid,
        rules: &RoleRules,
        ...
        match result {
            ...
            Ok(None) => Err(ServerError::QueryError(
                QueryError::UserNamespaceJunctionNotFound,
            )),
            Err(err) => Err(ServerError::from(ExternalError::from(err))),
        }
    ...
    pub async fn compare_user_namespace_perms(
        &self,
        requester_id: Uuid,
        target_id: Uuid,
        namespace_id: Uuid,
            ...
            (Ok(Some(requester)), Ok(Some(target))) => {
                ...
                match (requester_perms, target_perms) {
                    (Some(requester_role), Some(target_role)) => {
                        let requester_weight =
                            rules.role_rules.get(&requester_role).unwrap().weight;
    ...
    pub async fn get_namespace_members(
        ...
        let user_namespace_junction = junctions
            .into_iter()
            .map(|junc| (junc.user_id, junc.role))
            .collect::<HashMap<Uuid, String>>();

        let users_dto: Vec<MemberListDTO> = users
            .into_iter()
            .map(|user| MemberListDTO {
                id: user.id,
                email: user.email,
                username: user.username,
                role: user_namespace_junction
                    .get(&user.id)
                    .cloned()
